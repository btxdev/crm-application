...
function getBasicInfo() {
  fetch('php/app.php', {
    ...
    .then((data) => {
      console.log(data);
      if (data['status'] == 'OK') {
        document.querySelector('#header-username').innerHTML = data['username'];
...
function updateUsers() {
  sendData({
    ...
    let content = `
            <tr class="employees-table__title-row">
                    <td style="width: 20px;"><div class="title">#</div></td>
                    <td style="max-width: 60px;"><div class="title">Пользователь</div></td>
                    <td><div class="title">Имя</div></td>
                    <td><div class="title">Фамилия</div></td>
                    <td><div class="title">Отчество</div></td>
                    <td style="max-width: 50px;"><div class="title">Телефон</div></td>
                    <td><div class="title" style="width: 170px;">Почта</div></td>
    ...
    for (user of users) {
      content += `
                <tr>
                    ...
                    <td style="max-width: 60px;"><div class="field">${user['username']}</div></td>
                    ...
                    <td style="max-width: 50px;"><div class="field">${user['phone']}</div></td>
                    <td><div class="field" style="width: 170px;"><button onclick="sendMsg('${user['id']}', '${user['email']}')">Отправить письмо</button></div></td>
...
function sendMsg(id, email) {
  let result = confirm(
    `Пользователю #${id} будет отправлено письмо на адрес ${email} о готовности его заказа, продолжить?`
  );
  if (result) {
    sendData({
      body: {
